...
**Инлайн — в один вызов, без `/convert`.** Таблицу `.csv`/`.xlsx` можно подать **напрямую** как
`pzz_descriptions_file` в `POST /tasks/auto/chat/stream` (`mode=building_pzz_check`) — бэкенд
сконвертирует её на месте и добавит распознанное (число зон/ВРИ + warnings) в **ведущий `chunk`**
нарратива. Если обязательную колонку (`zone_code`/`vri_code`) определить не удалось — приходит
терминальный `error` + `done`, задача не создаётся (подать таблицу иначе или JSON).
