...
- `Топ5_возможных_ВРИ` — для not_allowed (pzz-check) или для любого объекта в classify-only: 5 ближайших вариантов

**`pzz_check` / `building_pzz_check`** (зоны ПЗЗ участвуют) дополнительно:
- `Код фактической зоны нахождения кадастра`, `Название фактической зоны нахождения кадастра`
- `Вердикт_ПЗЗ` — `allowed_main` / `allowed_conditional` / `allowed_auxiliary` / `not_allowed` / `unclear` / `no_actual_zone` / `no_zone_metadata`
- `Подобранный_ВРИ` + `Код_подобранного_ВРИ` (pzz_check); в `building_pzz_check` и проверке сценария — `Исходный_тип_объекта` (вместо `ВРИ_ЕГРН`) + `Тип_использования` + `Код_типа_использования` + `Основание_подбора_типа_использования` (с `output_version` v5; раньше — `ВРИ_ЕГРН` / `Подобранный_ВРИ` / `Код_подобранного_ВРИ` / `Основание_подбора_ВРИ`)

**`classify_only`** (зоны ПЗЗ НЕ участвуют вообще, начиная с `output_version` v4): вместо `Вердикт_ПЗЗ` —
...
| `warning` | `{ "message", "stage", "detail" }` | **НЕ фатально**: ответ сформирован и отдан, но **не сохранён в историю чата** (напр. истёк токен, `stage: "create_chat"`). Покажите мягкое уведомление (`message`), **не** трактуйте как ошибку сервиса. Ответ можно сохранить на своей стороне. `detail` — короткая техническая сводка (`"chat_storage returned 500"`), для логов, не для показа пользователю |
...
ПЗЗ (`Вердикт_ПЗЗ`, `Основание_подбора_типа_использования` и др.; см. D2), одна result-фича на каждую
