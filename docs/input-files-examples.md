...
> **Инлайн, без `/convert`:** ту же таблицу `.csv`/`.xlsx` можно подать напрямую как
> `pzz_descriptions_file` в `POST /tasks/auto/chat/stream` (`mode=building_pzz_check`) — конвертация
> произойдёт на месте, распознанное (зоны/ВРИ + warnings) попадёт в ведущий `chunk` нарратива, а при
> нераспознанной обязательной колонке придёт терминальный `error`. `/convert` нужен, когда маппинг
> колонок хочется проверить ДО запуска.
