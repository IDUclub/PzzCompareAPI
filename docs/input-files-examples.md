...
Как именно подобрано — видно в колонке результата `Основание_подбора_типа_использования`; неточное сопоставление
...
> **Имя зоны в результате.** В итоговом geojson и в ответе модели зона подписывается атрибутом
> «Название фактической зоны нахождения кадастра». Источник имени, по приоритету:
> **имя из описания (§4) → имя из этого слоя зон (`zone_name`) → код зоны**. То есть если в слое зон
> есть названия, а в описании их нет — в результат попадут названия из слоя (а не «Ж-1»). Чтобы
> читатель не разбирался в кодах, задавайте название хотя бы в одном из этих двух файлов.
...
> подобранные колонки таблицы и warnings) попадёт в ведущий `chunk` нарратива, а при нераспознанной
> обязательной колонке придёт терминальный `error`. `/convert` нужен, когда маппинг колонок хочется
