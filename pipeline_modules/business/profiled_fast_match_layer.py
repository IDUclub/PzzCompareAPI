...
SECTION_TO_VERDICT = {
    "main": "allowed_main",
    "conditional": "allowed_conditional",
    "auxiliary": "allowed_auxiliary",
}


def build_catalog_embed_text(vri_name: Any, vri_description: Any = "") -> str:
    """Build compact semantic text for embedding comparison."""
    return canonicalize_vri_name(
        f"{normalize_text(vri_name)} | {normalize_text(vri_description)}"
    )


def detect_direct_description_coverage(
    vri_text: str, candidate_name: Any, candidate_description: Any
) -> dict[str, Any]:
    ...
    desc_short_norm = build_short_description_match_text(
        candidate_description, max_tokens=DIRECT_DESC_MAX_TOKENS
    )
    ...
    best_metrics = (
        full_metrics
        if full_metrics["score"] >= short_metrics["score"]
        else short_metrics
    )
    phrase_hit = len(query_norm) >= 10 and query_norm in desc_full_norm
    is_direct = (
        best_metrics["coverage"] >= DIRECT_DESC_COVERAGE_THRESHOLD
        and best_metrics["overlap"]
        >= max(DIRECT_DESC_MIN_OVERLAP, min(4, len(tokenize_canonical(query_norm))))
        and (phrase_hit or best_metrics["seq_ratio"] >= DIRECT_DESC_SEQ_THRESHOLD)
    )
    return {
        "is_direct": bool(is_direct),
        "score": float(best_metrics["score"]),
        "coverage": float(best_metrics["coverage"]),
        "overlap": float(best_metrics["overlap"]),
        "seq_ratio": float(best_metrics["seq_ratio"]),
        "phrase_hit": bool(phrase_hit),
    }


SERVICE_HIERARCHY_PATH_CANDIDATES = [
    "data/services_hierarchy.json",
    "/mnt/data/services_hierarchy.json",
    "services_hierarchy.json",
]

PHYSICAL_HIERARCHY_PATH_CANDIDATES = [
    "data/physical_objects_hierarchy.json",
    "/mnt/data/physical_objects_hierarchy.json",
    "physical_objects_hierarchy.json",
]
...
def build_short_description_match_text(
    value: Any, max_tokens: int = FAST_DESC_MAX_TOKENS
) -> str:
...
def _family_from_service_path(path: tuple[str, ...]) -> Optional[str]:
    ...
    if (
        "здравоохран" in text
        or "амбулаторн" in text
        or "стационарн" in text
        or ("фармацевт" in text)
    ):
        return "healthcare"
    ...
    if (
        "услуг уход" in text
        or "парикмахер" in text
        or "салон красот" in text
        or ("бан" in text)
    ):
...
def _family_from_physical_path(path: tuple[str, ...]) -> Optional[str]:
    ...
    if not text:
        return None
    if (
        "электроснабж" in text
        or "электрическ подстанц" in text
        or "электростанц" in text
        or ("газорегуляторн пункт" in text)
        or ("подстанц" in text)
    ):
    ...
    if (
        "водоснабж" in text
        or "водозабор" in text
        or "насосн станц" in text
        or ("станц подкачк" in text)
        or ("подкачк воды" in text)
        or ("водонапорн башн" in text)
    ):
        return "communal_engineering"
    if "водоотведен" in text or "водоочистн" in text or "канализац" in text:
        return "communal_engineering"
    if (
        "линейн объект" in text
        or "линия электропередач" in text
        or "газопровод" in text
        or ("сеть теплоснабж" in text)
        or ("сеть водоотведен" in text)
        or ("сеть электросвяз" in text)
    ):
        return "communal_engineering"
    ...
    if "жилой дом" in text:
        return "residential"
    if (
        "уличн дорожн" in text
        or "тротуар" in text
        or "пешеходн переход" in text
        or ("проезд" in text)
        or ("набережн" in text)
        or ("бульвар" in text)
        or ("велодорож" in text)
    ):
...
def rank_zone_item_for_profile(item: pd.Series, profile: dict[str, Any]) -> int:
    ...
    if not candidate_keys or family == "unknown":
        return 0
    text_parts = [
        normalize_text(item.get("catalog_vri_name")),
        normalize_text(item.get("catalog_vri_description")),
        normalize_text(item.get("catalog_original_vri_name")),
        normalize_text(item.get("catalog_original_vri_description")),
    ]
...
def filter_zone_items_for_profile(
    zone_items: pd.DataFrame, profile: dict[str, Any]
) -> pd.DataFrame:
    ...
    work["__profile_rank__"] = work.apply(
        lambda row: rank_zone_item_for_profile(row, profile), axis=1
    )
    filtered = work.loc[work["__profile_rank__"] > 0].copy()
    if filtered.empty:
        return zone_items
    return filtered.sort_values(
        ["__profile_rank__", "section_name", "catalog_vri_name"],
...
def profile_allows_auto_decision(profile: dict[str, Any], candidate_name: Any) -> bool:
    ...
    if profile.get("is_residential") and (not profile.get("allow_residential_auto")):
        residential_markers = [
            "индивидуальн жилищн строительств",
            "блокированн",
            "малоэтажн",
            "среднеэтажн",
            "многоэтажн",
...
def try_explicit_description_coverage_in_zone(
    vri_text: Any, actual_zone_code: Any, context: Any = None
    ...
    if not query_norm:
        return None
    if not any(
        (re.search(pattern, query_norm) for pattern in rule.get("query_patterns", []))
    ):
        return None
    preferred_codes = expand_vri_codes_with_classifier_children(
        {normalize_text(code) for code in rule.get("preferred_codes", set())},
        classifier_children_map=(
            context.rosreestr_classifier_children_map if context is not None else {}
        ),
    )
    ...
    for _, item in zone_items.iterrows():
        ...
        candidate_text = canonicalize_vri_name(
        ...
        if not any(
            (
                ...
                for pattern in rule.get("candidate_patterns", [])
            )
        ):
            continue
        ...
        if any(
            (
                re.search(pattern, candidate_text)
                for pattern in rule.get("query_patterns", [])
            )
        ):
            score += 10
        candidates.append(
            {
                ...
                "verdict": verdict,
                "matched_vri_name": matched_vri_name,
                "matched_vri_code": matched_vri_code,
            }
        )
    if not candidates:
        return None
    best = sorted(
        ...
        key=lambda item: (
            item["score"],
            SECTION_PRIORITY.get(item["section_name"], 0),
            item["matched_vri_name"],
        ),
        reverse=True,
    )[0]
    reasons = {
        "communal_engineering": "Кадастровая формулировка покрывается описанием коммунального обслуживания в фактической зоне.",
        "street_network": "Кадастровая формулировка покрывается описанием ВРИ общего пользования / улично-дорожной сети в фактической зоне.",
        "household_outbuilding": "Кадастровая формулировка покрывается описанием вспомогательных / подсобных сооружений в фактической зоне.",
    }
    return {
        "verdict": best["verdict"],
        "matched_vri_name": best["matched_vri_name"],
        "matched_vri_code": best["matched_vri_code"],
        "reason": f"{reasons.get(family, 'Кадастровая формулировка покрывается описанием разрешенного ВРИ в фактической зоне.')} Опорный ВРИ: «{best['matched_vri_name']}» (код {best['matched_vri_code']}).",
    }
...
BRIDGE_BLOCK_PATTERNS = [
    "без\\s+вновь\\s+возводим(?:ых|ого)?\\s+окс",
    "без\\s+размещени[яй]\\s+нов(?:ых|ого)\\s+объект",
    "запреща(?:ет|ется)\\s+нов(?:ое|ые)\\s+строительств",
]
...
BRIDGE_EQUIVALENCE_HINT_PATTERNS = [
    "историческ",
    "культурн",
    "парк",
    "сквер",
...
def detect_zone_intent(
    zone_name: Any, zone_heading: Any, zone_summary: Any, base_zone_code: Any
) -> str:
    ...
    combined = canonicalize_vri_name(
        " ".join(
            [
                ...
                normalize_text(zone_heading),
                normalize_text(zone_summary),
            ]
        )
    )
    if not combined:
...
def find_bridge_evidence_phrase(
    intent: str, main_vri_full: Any, retrieval_text: Any
) -> Optional[str]:
    ...
    for part in [normalize_text(main_vri_full), normalize_text(retrieval_text)]:
        ...
        for raw_phrase in re.split("[\\n.;]", part):
            ...
            if any(
                (
                    re.search(pattern, canon_phrase)
                    for pattern in INTENT_SYNONYMS.get(intent, [])
                )
            ):
                return phrase[:220]
    return None
...
def has_bridge_conflict(main_vri_full: Any, retrieval_text: Any) -> bool:
    ...
    combined = canonicalize_vri_name(
        " ".join([normalize_text(main_vri_full), normalize_text(retrieval_text)])
    )
    return any((re.search(pattern, combined) for pattern in BRIDGE_BLOCK_PATTERNS))


def intents_match_for_bridge(
    zone_intent: str, object_intent: str, *, main_vri_full: Any, retrieval_text: Any
) -> bool:
    ...
    for group in BRIDGE_INTENT_EQUIVALENCE_GROUPS:
        if zone_intent in group and object_intent in group:
            combined = canonicalize_vri_name(
                " ".join(
                    [normalize_text(main_vri_full), normalize_text(retrieval_text)]
                )
            )
            if any(
                (
                    re.search(pattern, combined)
                    for pattern in BRIDGE_EQUIVALENCE_HINT_PATTERNS
                )
            ):
                return True
    return False
...
def try_semantic_bridge_autodecision(
    *,
    vri_text: Any,
    ...
    zone_intent = detect_zone_intent(
        zone_name, zone_heading, zone_summary, base_zone_code
    )
    object_intent = detect_object_intent(vri_text, profile)
    if zone_intent == "unknown" or object_intent == "unknown":
        return None
    if not intents_match_for_bridge(
        zone_intent,
        object_intent,
        main_vri_full=main_vri_full,
        retrieval_text=retrieval_text,
    ):
        return None
    evidence_phrase = find_bridge_evidence_phrase(
        object_intent, main_vri_full, retrieval_text
    )
    if not evidence_phrase or has_bridge_conflict(main_vri_full, retrieval_text):
        return None
    return {
        "verdict": "allowed_main",
        "matched_vri_name": evidence_phrase,
        "matched_vri_code": pd.NA,
        "reason": f"Решение принято по semantic bridge-правилу: zone_intent={zone_intent}; object_intent={object_intent}; подтверждение: «{evidence_phrase}».",
