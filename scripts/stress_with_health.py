...
tok = [
    ...
    for l in (ROOT / ".env.development").read_text(encoding="utf-8").splitlines()
    if l.startswith("URBAN_API_TOKEN=")
][0]
...
def health_poll():
    while not stop.is_set():
        ...
        try:
            ...
            health.append(
                [round(time.time() - t0, 1), r.status_code, round(time.time() - t, 2)]
            )
...
(ROOT / "scripts" / "_stress_fix.json").write_text(
