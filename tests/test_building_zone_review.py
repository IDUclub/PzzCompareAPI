...
def test_numeric_zones_proceed_not_approximate(tmp_path) -> None:
    ...
    r = review_building_zones(
        ...
        code_col="zone_code",
        name_col=None,
        user_uploaded_descriptions=False,
        confirmed_map=None,
        template_path=_template(tmp_path, _entry("Ж-1", "Жилая", "2.1")),
        threshold=5,
    )
...
def test_user_descriptions_proceed_not_approximate(tmp_path) -> None:
    r = review_building_zones(
        zones_fc=_zones("Ж-1"),
        code_col="Индекс_зоны",
        name_col=None,
        user_uploaded_descriptions=True,
        confirmed_map=None,
        template_path=_template(tmp_path, _entry("Ж-1", "Жилая", "2.1")),
        threshold=5,
    )
...
def test_confirmed_map_proceeds_and_stays_approximate(tmp_path) -> None:
    r = review_building_zones(
        zones_fc=_zones("Ж-1", "СХ-3"),
        code_col="Индекс_зоны",
        name_col=None,
        user_uploaded_descriptions=False,
        confirmed_map={"СХ-3": "Ж-1"},
        template_path=_template(tmp_path, _entry("Ж-1", "Жилая", "2.1")),
        threshold=5,
    )
...
def test_build_confirmed_overlay_maps_user_code_to_template_vri(tmp_path) -> None:
    path = _template(
        tmp_path, _entry("Ж-1", "Жилая", "2.1"), _entry("АГ-1", "Сельхоз", "1.15")
    )
...
def test_suggestion_prompt_enums_candidates_and_parses_hits() -> None:
    uncovered = [
        UncoveredZone("СХ-3", "сельхоз", "сх3"),
        UncoveredZone("Т-1", "транспорт", "т1"),
    ]
