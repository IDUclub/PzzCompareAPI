...
### Разделение параметров (важно)

Часть контекста агенту **не видна** и приходит «в обход» LLM:

- `scenario_id` — из `_meta` запроса (кладёт фронтенд);
- Bearer-токен пользователя — из заголовка `Authorization` (форвардится в urban_api).

Видимые агенту параметры — только содержательные: `year`, `source`,
`physical_object_type_id` (4 = жилые), `group_by` и т.п.

### Типовой диалог

```
get_scenario_zones_info(year, source)            # справка + проверка доступа/токена
classify_scenario(year, source, type_id) → external_id
poll get_scenario_classification_status(external_id) → finished
get_scenario_classification_report(external_id)  # отдать chat_message пользователю
```
